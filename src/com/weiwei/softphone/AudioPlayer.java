...
public class AudioPlayer {
	...
	public AudioPlayer() {
		mAudioManager = ((AudioManager) VsApplication.getContext().getSystemService(Context.AUDIO_SERVICE));
		mVibrator = (Vibrator) VsApplication.getContext().getSystemService(Context.VIBRATOR_SERVICE);
	}
	...
	public synchronized void startRinging() {
		...
		try {
			...
			mRingerPlayer.setDataSource(VsApplication.getContext(),
	...
	public synchronized void playerAlertAudio(int resourceId) {
		MediaPlayer mp = MediaPlayer.create(VsApplication.getContext(), resourceId);
		try {
			mp.prepare();
	...
	public void startRingBefore180Player(final int rId,final boolean isLoop) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				AssetFileDescriptor afd = VsApplication.getContext().getResources()
	...
	public synchronized void setPlayoutSpeaker(boolean loudspeakerOn, int index) {
		...
		if ((3 == apiLevel) || (4 == apiLevel)) {
			...
			if ((android.os.Build.BRAND.equals("Samsung") || android.os.Build.BRAND.equals("samsung"))
				...
				if (VsUserConfig.getDataBoolean(VsApplication.getContext(), VsUserConfig.JKey_DIALTESTMODELINCALL,
						false)) {
					mAudioManager.setMode(AudioManager.MODE_IN_CALL);
				} else {
					if (isSetMode()) {
