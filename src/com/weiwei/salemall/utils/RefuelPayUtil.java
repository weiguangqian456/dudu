...
import com.weiwei.base.activity.aplpay.PayResult;
...
public class RefuelPayUtil {
    ...
    private static RefuelPayUtil instance;
    private static Context mContext;
    private int flag;

    private RefuelPayUtil() {
    }

    public static RefuelPayUtil getInstance(Context context) {
        if (instance == null) {
            instance = new RefuelPayUtil();
        }
        mContext = context;
        return instance;
    }
...
//                    PayResult payResult = new PayResult((String) msg.obj);
    ...
    public void toWXPayCom(Context mContext,
                           String result) {
        VsApplication.getInstance().payType = 100;
        //result中是重服务器请求到的签名后各个字符串，可自定义
        //result是服务器返回结果
        IWXAPI api = WXAPIFactory.createWXAPI(mContext, DfineAction.WEIXIN_APPID,true);

        PayReq request = new PayReq();
        request.appId = WX_APP_ID;          //appID
        request.partnerId = WX_MCH_ID;   //商户ID
...
//        }
    ...
    public void toWXPay(Context mContext, String result) {
        ...
        PayReq request = new PayReq();
        request.appId = DfineAction.WEIXIN_APPID;          //appID
        request.partnerId = DfineAction.WEIXIN_MCH_ID;   //商户ID
        request.prepayId = result;
        request.packageValue = "Sign=WXPay";
        request.nonceStr = genNonceStr();
        request.timeStamp = "" + (System.currentTimeMillis() / 1000);
