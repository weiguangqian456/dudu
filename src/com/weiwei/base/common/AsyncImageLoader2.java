...
public class AsyncImageLoader2 {
	 ...
	 * 内存图片软引用缓冲
	 */
	public static HashMap<String, SoftReference<Bitmap>> imageCache = null;
	 ...
	 */
	public Bitmap loadDrawableByTag(final TagInfo tag, final ImageCallBack callback, final int width, final int height) {
		// 如果是富媒体图片 文件名以msgid+index方式保存
		final String fileName = (tag.getMsgid() + "" + tag.getPosition())
				+ (tag.getUrl().substring(tag.getUrl().lastIndexOf(".")).length() > 3 ? tag.getUrl().substring(
						tag.getUrl().lastIndexOf(".")) : ".png");
		...
		if (file.exists()) {
			...
			// imgData.setUri(uri);
			imgData.setBitmap(BitmapFactory.decodeFile(ALBUM_PATH + fileName));
			imageCache.put(fileName, new SoftReference<Bitmap>(imgData.getBitmap()));
			return imgData.getBitmap();
		}
		...
		BaseRunable newRunable = new BaseRunable() {
			public void run() { // 线程运行主体
				TagInfo info = getDrawableIntoTag(tag, fileName, width, height);
	 ...
	 */
	public TagInfo getDrawableIntoTag(TagInfo info, String fileName, int width, int height) {
		...
		try {
			...
			if (conn.getResponseCode() == HttpURLConnection.HTTP_OK) {
				...
				Bitmap bitmap = VsUtil.zoomImg(BitmapFactory.decodeStream(is), width, height);
	 ...
	 * @throws IOException
	 */
	private Uri saveFile(Bitmap bitmap, String fileName) throws IOException {
