...
public class Notifier {
    ...
    protected final static String[] msg_eng = {"sent a message", "sent a picture", "sent a voice", "sent location message", "sent a video", "sent a file", "%1 contacts sent %2 "
            + "messages"};
    ...
    public void sendNotification(String message) {
        try {
            ...
            Notification notification = new NotificationCompat.Builder(appContext).setSmallIcon(appContext.getApplicationInfo().icon).setSound(defaultSoundUri).setWhen(System
                    .currentTimeMillis()).setAutoCancel(true).setContentTitle(contentTitle).setTicker(message).setContentText(message).setContentIntent(pendingIntent).build();
    ...
    protected void sendNotification(Message message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            ...
            NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(appContext).setSmallIcon(appContext.getApplicationInfo().icon).setWhen(System.currentTimeMillis
                    ()).setAutoCancel(true);
